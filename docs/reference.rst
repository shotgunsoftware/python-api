...
Used to specify a path to an external SSL certificates file.  This environment variable can be used in place of the ``ca_certs`` keyword argument to the :class:`~shotgun.Shotgun` constructor.  In the case that both this environment variable is set and the keyword argument is provided, the value from the keywork argument will be used.


SHOTGUN_API_RETRY_INTERVAL
==========================

Stores the number of milliseconds to wait between request retries.  By default, a value of 3000 milliseconds is used. You can override this default either by setting this environment variable, or by setting the ``rpc_attempt_interval`` property on the config like so: ::
